...
export const DropDown = <T,>(props: DropDownProps<T>): JSX.Element => {
    ...
    const form = useMemo(() => {
        console.log("[DropDown] render!", props.title);
        return (
...
export const Toggle: FC<SwitchProps> = ({ title, current, onchange }: SwitchProps) => {
    const classes = useStyles();
    const form = useMemo(() => {
...
export const SingleValueSlider: FC<SingleValueSliderProps> = ({ title, current, min, max, step, onchange }: SingleValueSliderProps) => {
    const classes = useStyles();
    const form = useMemo(() => {
...
export const VideoInputSelect: FC<VideoInputSelectProps> = ({ title, current, options, onchange, cameraResolutions }: VideoInputSelectProps) => {
    ...
    const form = useMemo(() => {
        ...
        const onchangeInternal = async (val: string) => {
            ...
            if (val === "File") {
                ...
                currentMediaStream?.getTracks().forEach((tr) => {
                    tr.stop();
                });
            ...
            <FormControl className={classes.formControl}>
                ...
                {targetType === "Camera" ? (
                    <Select
                        ...
                        onChange={(e: any) => {
                            onchangeTargetResolution(e.target.value);
                        }}
                    >
    ...
    }, [current, targetType, options, targetCamera, targetResolutionKey, currentMediaStream]); // eslint-disable-line
    return <>{form}</>;
};
...
export const FileChooser: FC<FileChooserProps> = ({ title, onchange }: FileChooserProps) => {
    const classes = useStyles();
