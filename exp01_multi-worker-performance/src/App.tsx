...
import { AsciiOperatipnParams } from '@dannadori/asciiart-worker-js/dist/const';
import { FaceSwap } from './Faceswap/Faceswap';
import { VirtualBackground } from './VirtualBackground/VirtualBackground';
import {Mutex} from 'await-semaphore';
export const EFFECT = {"none":"none", "image":"image", "ascii":"ascii", "canny":"canny"}
type EFFECT = keyof typeof EFFECT;

class App extends DemoBase {
  private mutex = new Mutex()
  private faceswap: FaceSwap = new FaceSwap(640, 480)
  private maskImage = document.createElement("img")
  private backgroundImage = document.createElement("img")
  private bgEffect       = EFFECT.none
  private fgEffect       = EFFECT.none
  private vbg = new VirtualBackground()  
  private faceswapEnable = true
  private handSkeletonEnbale = false
  private facemeshForMask = (()=>{
    const m = new FacemeshWorkerManager()
    const config = generateFacemeshDefaultConfig()
    m.init(config).then(()=>{
      console.log("facemesh for mask is loaded.")
      this.selectMaskImage(this.IMAGE_PATH)
    })
    return m
  })()
  private facemeshForMaskParams = (()=>{
    const p = generateDefaultFacemeshParams()
    p.processHeight = -1
    p.processWidth = -1
    p.predictIrises = true
    return p
  } )()
...
//      config.processOnLocal = true
    ...
    (()=>{
      const config = generateOpenCVDefaultConfig()
//      config.processOnLocal = true
      return config
    })(),
    (()=>{
...
//      config.processOnLocal = true
  ...
  paramss = [
    ...
    (()=>{
      const params = generateDefaultOpenCVParams()
      return params
    })(),
    (()=>{
  ...
  bodypixCanvas: HTMLCanvasElement = (()=>{
  ...
  opencvCanvas: HTMLCanvasElement = (()=>{
    const newCanvas = document.createElement("canvas")
    // newCanvas.style.display="none"
    document!.getRootNode()!.lastChild!.appendChild(newCanvas)
    return newCanvas
  ...
  handposeCanvas: HTMLCanvasElement = (()=>{
    const newCanvas = document.createElement("canvas")
    // newCanvas.style.display="none"
    document!.getRootNode()!.lastChild!.appendChild(newCanvas)
    return newCanvas
  ...
  private selectMaskImage(path:string){
    this.maskImage.onload = () =>{
      this.maskCanvas.width = this.maskImage.width
      this.maskCanvas.height = this.maskImage.height
      const ctx = this.maskCanvas.getContext("2d")!
      // ctx.drawImage(this.maskImage, 0, 0, this.maskCanvas.width, this.maskCanvas.height)
      ctx.drawImage(this.maskImage, 0, 0, this.maskCanvas.width, this.maskCanvas.height)
      this.setMaskImage()
    }
    this.maskImage.src = path
  }

  originalCanvasSizeChanged = (width:number, height:number) =>{
    this.faceswap = new FaceSwap(width, height)
    this.setMaskImage()
  }
  getCustomMenu = () =>{
    const menu:ControllerUIProp[]= [
      ...
      },
      {
        ...
        callback: (val:string|number|MediaStream)=>{
          ...
          this.configs[0].processOnLocal     = (processOnLocal === "on" ? true  : false) as boolean
          this.configs[1].processOnLocal     = (processOnLocal === "on" ? true  : false) as boolean
          this.configs[2].processOnLocal     = (processOnLocal === "on" ? true  : false) as boolean
          this.configs[3].processOnLocal     = (processOnLocal === "on" ? true  : false) as boolean
          this.configs[4].processOnLocal     = (processOnLocal === "on" ? true  : false) as boolean
          this.requireReload()
        },
      },
      {
        ...
        range: [0, 1, 0.1],
        callback: (val: string | number | MediaStream) => {
          (this.paramss[0] as BodyPixOperatipnParams).segmentPersonParams!.segmentationThreshold = val as number
        },
      },
      {
        title: "fontSize",
        ...
        range: [2, 20, 1],
        callback: (val: string | number | MediaStream) => { 
          (this.paramss[2] as AsciiOperatipnParams).fontSize = val as number
        },
      },    
      {
        title: "ProcessWidth",
        ...
        range: [300, 1024, 10],
        callback: (val: string | number | MediaStream) => {
          this.paramss[0].processWidth = val as number
          this.paramss[1].processWidth = val as number
          this.paramss[2].processWidth = val as number
          this.paramss[3].processWidth = val as number
        },
      },
      {
        title: "ProcessHeight",
        ...
        range: [300, 1024, 10],
        callback: (val: string | number | MediaStream) => {
          this.paramss[0].processHeight = val as number
          this.paramss[1].processHeight = val as number
          this.paramss[2].processHeight = val as number
          this.paramss[3].processHeight = val as number
        },
      },
      {
        title: "mask",
      ...
      },
      {
        ...
        callback: ((val: string | number | MediaStream) => {
          this.bgEffect=val as EFFECT
        })
      },
      {
        title: "foreground",
        currentIndexOrValue: 0,
        values: ["none", "canny", "ascii"],
        callback: ((val: string | number | MediaStream) => {
          this.fgEffect=val as EFFECT
        })
      },
      {
        title: "maskEnable",
        ...
        values: ["on", "off"],
        callback: ((val: string | number | MediaStream) => {
          this.faceswapEnable = val === "on" ? true : false
        })
      },
      {
        title: "face_countour_aplha",
        currentIndexOrValue: 1,
        range: [0, 1, 0.01],
        callback: ((val: string | number | MediaStream) => {
          this.faceswap.face_countour_alpha = val as number
        })
      },
      {
        title: "mask_color_brightness",
        currentIndexOrValue: 1,
        range: [0, 1, 0.01],
        callback: ((val: string | number | MediaStream) => {
          this.faceswap.mask_color_brightness = val as number
        })
      },
      {
        title: "mask_color_alpha",
        currentIndexOrValue: 1,
        range: [0, 1, 0.01],
        callback: ((val: string | number | MediaStream) => {
          this.faceswap.mask_color_alpha = val as number
        })
      },
      {
        title: "iris_enable",
        ...
        values: ["on", "off"],
        callback: ((val: string | number | MediaStream) => {
          (this.paramss[1] as FacemeshOperatipnParams).predictIrises = val === "on" ? true : false
        })
      },
      {
        title: "handSkelton",
        ...
        values: ["on", "off"],
        callback: ((val: string | number | MediaStream) => {
          this.handSkeletonEnbale = val === "on" ? true : false
        })
      },
    ]
    return menu
  ...
  drawBodypixRes = (prediction:SemanticPersonSegmentation, canvas:HTMLCanvasElement) =>{
    ...
    ctx.drawImage(this.tmpCanvas, 0, 0, canvas.width, canvas.height)
    ctx.globalCompositeOperation = 'source-in';
    ctx.drawImage(this.originalCanvas.current!, 0, 0, canvas.width, canvas.height)
  }


  // drawFacemeshRes = (prediction: AnnotatedPrediction[], canvas:HTMLCanvasElement, params:any) =>{
  //   const ctx = canvas.getContext("2d")!
  ...
  //     const scaleX = canvas.width / params.processWidth
  //     const scaleY = canvas.height / params.processHeight
  ...
  //       ].map(index =>
  //          [
  //           // (keypoints[index][0] / params.processWidth)  * canvas.width,
  //           // (keypoints[index][1] / params.processHeight) * canvas.height,
  //           (keypoints[index][0] * scaleX),
  //           (keypoints[index][1] * scaleY),
  //        ]);
  ...
  drawAsciiRes = (convertedImage:HTMLCanvasElement, canvas:HTMLCanvasElement) =>{
    const ctx = canvas.getContext("2d")!
    ctx.drawImage(convertedImage, 0, 0, canvas.width, canvas.height)
  }

  drawOpencvRes = (convertedImage:HTMLCanvasElement, canvas:HTMLCanvasElement) =>{
    const ctx = canvas.getContext("2d")!
    ctx.drawImage(convertedImage, 0, 0, canvas.width, canvas.height)
  }

  drawHandposeRes = (prediction:any[], canvas:HTMLCanvasElement, params:any) =>{
    const ctx = canvas.getContext("2d")!
  ...
  debugview = (prediction:any[]) =>{
    this.bodypixCanvas.width = this.resultCanvasRef.current!.width /2
    this.bodypixCanvas.height = this.resultCanvasRef.current!.height /2
    this.facemeshCanvas.width = this.resultCanvasRef.current!.width /2
    this.facemeshCanvas.height = this.resultCanvasRef.current!.height /2
    this.asciiCanvas.width = this.resultCanvasRef.current!.width /2
    this.asciiCanvas.height = this.resultCanvasRef.current!.height /2
    this.opencvCanvas.width = this.resultCanvasRef.current!.width /2
    this.opencvCanvas.height = this.resultCanvasRef.current!.height /2
    this.handposeCanvas.width = this.resultCanvasRef.current!.width /2
    this.handposeCanvas.height = this.resultCanvasRef.current!.height /2

    this.drawBodypixRes(prediction[0], this.bodypixCanvas)
    // this.drawFacemeshRes(prediction[1], this.facemeshCanvas, this.paramss[1])
    this.drawAsciiRes(prediction[2], this.asciiCanvas)
    this.drawOpencvRes(prediction[3], this.opencvCanvas)
    this.drawHandposeRes(prediction[4], this.handposeCanvas, this.paramss[4])
  }

  showSwapCanvas = (result:HTMLCanvasElement) => {
    this.swapCanvas.width = this.resultCanvasRef.current!.width /2
    this.swapCanvas.height = this.resultCanvasRef.current!.height /2
    this.swapCanvas.getContext("2d")!.fillRect(0, 0, this.swapCanvas.width, this.swapCanvas.height)
    this.swapCanvas.getContext("2d")!.drawImage(result, 0, 0, this.swapCanvas.width, this.swapCanvas.height)
  }


  handleResult = (prediction: any[]) => {
    this.debugview(prediction)
    // console.log("prediction",prediction)
    // console.log(this.bgEffect, this.fgEffect, this.faceswapEnable)

    const thisTimeBackground = (()=>{
      switch(this.bgEffect){
        case EFFECT.none:
          return this.originalCanvas.current!
        case EFFECT.image:
    ...
    const thisTimeForeground = (()=>{
      switch(this.fgEffect){
        case EFFECT.none:
          return this.originalCanvas.current!
        case EFFECT.ascii:
          return prediction[2]
    ...
    const vbgResultCanvas = this.vbg.convert(thisTimeForeground, thisTimeBackground, prediction[0])

    if(this.faceswapEnable === true){
      const scaleX = this.originalCanvas.current!.width / this.paramss[1].processWidth
      const scaleY = this.originalCanvas.current!.height / this.paramss[1].processHeight
      const swapResult = this.faceswap.swapFace(this.originalCanvas.current!, prediction[1], scaleX, scaleY)
      this.showSwapCanvas(swapResult)
      vbgResultCanvas.getContext("2d")?.drawImage(swapResult, 0, 0, vbgResultCanvas.width, vbgResultCanvas.height)
    }

    this.resultCanvasRef.current!.getContext("2d")!.drawImage(vbgResultCanvas, 0, 0, this.resultCanvasRef.current!.width, this.resultCanvasRef.current!.height)

    if(this.handSkeletonEnbale){
      this.drawHandposeRes(prediction[4], this.resultCanvasRef.current!, this.paramss[4])
    }
