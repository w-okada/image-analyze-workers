    ...
    predict = async (config: HandPoseDetectionConfig, params: HandPoseDetectionOperationParams, targetCanvas: HTMLCanvasElement): Promise<Hand[] | null> => {
        if (config.modelType === ModelTypes.mediapipe || config.modelType === ModelTypes.tfjs) {
            ...
            for (let i = 0; i < handNum; i++) {
                ...
                // -> 12 + 8 + 14 + 42 = 76
                const offset = this.tfliteOutputAddress / 4 + 1 + i * (12 + 8 + 14 + 42)
                ...
                for (let j = 0; j < 4; j++) {
                    let rotatedOffset = (this.tfliteOutputAddress / 4 + 1) + (i * 76) + (12) + (j * 2)
                ...
                for (let j = 0; j < 7; j++) {
                    let palmKeypointOffset = (this.tfliteOutputAddress / 4 + 1) + (i * 76) + (12 + 8) + (j * 2)
                ...
                for (let j = 0; j < 21; j++) {
                    let landmarkKeypointOffset = (this.tfliteOutputAddress / 4 + 1) + (i * 76) + (12 + 8 + 14) + (j * 2)
