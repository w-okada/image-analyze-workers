...
//     options: {
//         posemask: ApplicationModes.posemask,
...
//     },
// };
// const maxposesSliderProps: CommonSliderProps = {
//     id: "max-poses-slider",
//     title: "max poses",
//     currentValue: config.maxposes,
//     max: 10,
//     min: 1,
//     step: 1,
//     width: "30%",
//     onChange: (value: number) => {
//         config.maxposes = value;
//         setConfig({ ...config });
//     },
//     integer: false,
// };
...
return (
    <div style={{ display: "flex", flexDirection: "column" }}>
        ...
        {/* <CommonSlider {...maxposesSliderProps}></CommonSlider> */}
        ...
        const render = async () => {
            ...
            try {
                ...
                if (snap.width > 0 && snap.height > 0) {
                    ...
                    dstCtx.fillStyle="#ff0000";
                    // console.log("RES:",res)

                    res.forEach(pose=>{
                        if(pose.score < 0.01 || pose.landmarkScore < 0.01){
                            return
                        }
                        const score =pose.score.toFixed(2)
                        const lscore = pose.landmarkScore.toFixed(2)
                        dstCtx.fillStyle="#000000";
                        dstCtx.font = 'bold 48pt sans-serif';
                        dstCtx.fillText(`${score}/${lscore}`, (pose.pose.minX)*dst.width, (pose.pose.minY)*dst.height)

                        dstCtx.fillStyle="#ff0000aa";
                        dstCtx.fillRect(                        
                        (pose.pose.minX)*dst.width, 
                        (pose.pose.minY)*dst.height, 
                        (pose.pose.maxX - pose.pose.minX)*dst.width, (pose.pose.maxY - pose.pose.minY) *dst.height)

                        dstCtx.fillStyle="#00ff00aa";
                        dstCtx.fillRect(                        
                            (pose.poseWithMargin.minX)*dst.width, 
                            (pose.poseWithMargin.minY)*dst.height, 
                            (pose.poseWithMargin.maxX - pose.poseWithMargin.minX)*dst.width, 
                            (pose.poseWithMargin.maxY - pose.poseWithMargin.minY) *dst.height
                        )
                        dstCtx.fillStyle="#0000ffff";
                        pose.rotatedPose.positions.forEach(p=>{
                            dstCtx.fillRect(p.x*dst.width, p.y*dst.height, 20,20)
                        })

                        dstCtx.fillStyle="#00ffffaa";
                        pose.poseKeypoints.forEach((p,index)=>{
                            dstCtx.fillRect(p.x*dst.width, p.y*dst.height, 30,30)
                            dstCtx.fillText(`-->${index}`, p.x*dst.width, p.y*dst.height)
                        })

                        dstCtx.fillStyle="#ffffffaa";
                        pose.landmarkKeypoints.forEach(p=>{
                            if(p.presence < 0.8){
                                dstCtx.fillStyle="#ff0000aa";
                            }else if(p.visibility < 0.8){
                                dstCtx.fillStyle="#00ff00aa";
                            }else{
                                dstCtx.fillStyle="#0000ffaa";
                            }
                        ...
                        dstCtx.beginPath();
                        const offset=0
                        dstCtx.moveTo(pose.landmarkKeypoints[offset+11].x*dst.width,pose.landmarkKeypoints[offset+11].y*dst.height);
