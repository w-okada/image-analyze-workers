#ifndef __FACE_CORE_HPP__
#define __FACE_CORE_HPP__
...
#include <list>
#include "face.hpp"
#include "mediapipe_face/Anchor.hpp"
#include "mediapipe_face/KeypointDecoder.hpp"
#include "mediapipe_face/NonMaxSuppression.hpp"
#include "mediapipe_face/PackFaceResult.hpp"
#include "const.hpp"
std::unique_ptr<tflite::Interpreter> faceInterpreter;
std::unique_ptr<tflite::Interpreter> faceLandmarkInterpreter;
...
public:
    ...
    // Detector
    ////////////////////////////////////
    char *faceDetectorModelBuffer;
    void initFaceDetectorModelBuffer(int size)
    {
        faceDetectorModelBuffer = new char[size];
    }
    char *getFaceDetectorModelBufferAddress()
    {
        return faceDetectorModelBuffer;
    }
    int loadFaceDetectorModel(int size)
    {
        ...
        builder(&faceInterpreter);
        CHECK_TFLITE_ERROR(faceInterpreter != nullptr);
        CHECK_TFLITE_ERROR(faceInterpreter->AllocateTensors() == kTfLiteOk);

        printf("[WASM]: Model Info");

        printf("[WASM]: INTPUT NUM: %lu\n", faceInterpreter->inputs().size());
        for (auto i : faceInterpreter->inputs())
        {
            const TfLiteTensor *tensor = faceInterpreter->tensor(i);
        ...
        printf("[WASM]: OUTTPUT NUM: %lu\n", faceInterpreter->outputs().size());
        for (auto i : faceInterpreter->outputs())
        {
            const TfLiteTensor *tensor = faceInterpreter->tensor(i);
        ...
        int output_num = faceInterpreter->outputs().size();
        for (int i = 0; i < output_num; i++)
        {
            int tensor_idx = faceInterpreter->outputs()[i];
            const char *tensor_name = faceInterpreter->tensor(tensor_idx)->name;
            if (strcmp(tensor_name, "regressors") == 0 || strcmp(tensor_name, "reshaped_regressor_face_4") == 0 || strcmp(tensor_name, "Identity") == 0)
            {
                points_ptr = faceInterpreter->typed_output_tensor<float>(i);
            ...
            {
                scores_ptr = faceInterpreter->typed_output_tensor<float>(i);
            }
            else
            {
        ...
        face_generate_ssd_anchors(&s_anchors, detectorType);
        return 0;
    }
    ...
    // Landmark
    ////////////////////////////////////
    char *faceLandmarkModelBuffer;
    void initFaceLandmarkModelBuffer(int size)
    {
        faceLandmarkModelBuffer = new char[size];
    }
    char *getFaceLandmarkModelBufferAddress()
    {
        return faceLandmarkModelBuffer;
    }
    ...
    int loadFaceLandmarkModel(int size)
    {
        ...
        builder(&faceLandmarkInterpreter);
        CHECK_TFLITE_ERROR(faceLandmarkInterpreter != nullptr);
        CHECK_TFLITE_ERROR(faceLandmarkInterpreter->AllocateTensors() == kTfLiteOk);

        printf("[WASM]: Model Info");

        printf("[WASM]: INTPUT NUM: %lu\n", faceLandmarkInterpreter->inputs().size());
        for (auto i : faceLandmarkInterpreter->inputs())
        {
            const TfLiteTensor *tensor = faceLandmarkInterpreter->tensor(i);
            printf("[WASM]: INTPUT[%d]: Name:%s Size:%zu\n", i, tensor->name, tensor->bytes);
            printf("[WASM]:    TYPE(1:float):%d\n", tensor->type);
            int num = tensor->dims->size;
            printf("[WASM]:    SHAPE:[");
        ...
        printf("[WASM]: OUTTPUT NUM: %lu\n", faceLandmarkInterpreter->outputs().size());
        for (auto i : faceLandmarkInterpreter->outputs())
        {
            const TfLiteTensor *tensor = faceLandmarkInterpreter->tensor(i);
            printf("[WASM]: OUTTPUT[%d]: Name:%s Size:%zu\n", i, tensor->name, tensor->bytes);
            printf("[WASM]:    TYPE(1:float):%d\n", tensor->type);
        ...
        int output_num = faceLandmarkInterpreter->outputs().size();

        for (int j = 0; j < output_num; j++)
        {
            int tensor_idx = faceLandmarkInterpreter->outputs()[j];
            const char *tensor_name = faceLandmarkInterpreter->tensor(tensor_idx)->name;
            if (strcmp(tensor_name, "output_mesh_identity") == 0 || strcmp(tensor_name, "conv2d_21") == 0 || strcmp(tensor_name, "output_mesh_identity:0") == 0)
            {
                landmark_ptr = faceLandmarkInterpreter->typed_output_tensor<float>(j);
            ...
            {
                faceflag_ptr = faceLandmarkInterpreter->typed_output_tensor<float>(j);
            }
            else if (strcmp(tensor_name, "output_lips") == 0 || strcmp(tensor_name, "output_lips:0") == 0)
            {
                output_lips_ptr = faceLandmarkInterpreter->typed_output_tensor<float>(j);
            }
            else if (strcmp(tensor_name, "output_left_eye") == 0 || strcmp(tensor_name, "output_left_eye:0") == 0)
            {
                output_left_eye_ptr = faceLandmarkInterpreter->typed_output_tensor<float>(j);
            }
            else if (strcmp(tensor_name, "output_right_eye") == 0 || strcmp(tensor_name, "output_right_eye:0") == 0)
            {
                output_right_eye_ptr = faceLandmarkInterpreter->typed_output_tensor<float>(j);
            }
            else if (strcmp(tensor_name, "output_left_iris") == 0 || strcmp(tensor_name, "output_left_iris:0") == 0)
            {
                output_left_iris_ptr = faceLandmarkInterpreter->typed_output_tensor<float>(j);
            }
            else if (strcmp(tensor_name, "output_right_iris") == 0 || strcmp(tensor_name, "output_right_iris:0") == 0)
            {
                output_right_iris_ptr = faceLandmarkInterpreter->typed_output_tensor<float>(j);
            }
            else
            {
    ...
    unsigned char *faceInputBuffer;
    void initFaceInputBuffer(int width, int height, int channel)
    {
        faceInputBuffer = new unsigned char[width * height * channel];
        initFaceOutputBuffer();
        initFaceTemporaryBuffer();
    }
    unsigned char *getFaceInputBufferAddress()
    {
        return faceInputBuffer;
    }

    float *faceOutputBuffer;
    void initFaceOutputBuffer()
    {
        faceOutputBuffer = new float[1024 * 1024];
    }
    float *getFaceOutputBufferAddress()
    {
        return faceOutputBuffer;
    }

    unsigned char *faceTemporaryBuffer;
    void initFaceTemporaryBuffer()
    {
        faceTemporaryBuffer = new unsigned char[1024 * 1024 * 4];
    }
    unsigned char *getFaceTemporaryBufferAddress()
    {
        return faceTemporaryBuffer;
    }

    void execFace(int width, int height, int max_face_num)
    {
        float *input = faceInterpreter->typed_input_tensor<float>(0);

        cv::Mat inputImage(height, width, CV_8UC4, faceInputBuffer);
        cv::Mat temporaryImage(1024, 1024, CV_8UC4, faceTemporaryBuffer);
        ...
        // // (2) Infer
        // printf("Infer!\n");
        CHECK_TFLITE_ERROR(faceInterpreter->Invoke() == kTfLiteOk);
        ...
        for (int i = 0; i < face_result.num; i++)
        {
            ...
            float *landmarkInput = faceLandmarkInterpreter->typed_input_tensor<float>(0);
            ...
            //// Landmark検出
            CHECK_TFLITE_ERROR(faceLandmarkInterpreter->Invoke() == kTfLiteOk);
        ...
        //// output
        *faceOutputBuffer = 0.0; // 検出した顔の数を初期化
        float *currentOutputPosition = faceOutputBuffer + 1;
        if (face_result.num > 0)
        {
            for (int i = 0; i < face_result.num; i++)
            {

                (*faceOutputBuffer)++; // 検出した手の数をインクリメント
                // score, rotateion
