...
class App extends DemoBase {
  ...
  getCustomMenu = () =>{
    const menu:ControllerUIProp[]= [
      ...
      },
      {
        ...
        range: [2, 20, 1],
        callback: (val: string | number | MediaStream) => { 
          (this.paramss[1] as AsciiOperatipnParams).fontSize = val as number
        },
      },    
      {
        title: "ProcessWidth",
        ...
        callback: (val: string | number | MediaStream) => {          
          ...
          // @ts-ignore
          //this.paramss[4].processWidth = val as number
        },
      },
      {
        title: "ProcessHeight",
        ...
        callback: (val: string | number | MediaStream) => {
          ...
          // @ts-ignore
          // this.paramss[4].processHeight = val as number
        },
      },
      {
        title: "mask",
  ...
  drawSwapFace = (prediction: AnnotatedPrediction[], canvas:HTMLCanvasElement, params:any) =>{
    //@ts-ignore
    const scaleX = this.originalCanvas.current!.width / params.processWidth
    //@ts-ignore
    const scaleY = this.originalCanvas.current!.height / params.processHeight
    const swapResult = this.faceswap.swapFace(this.originalCanvas.current!, prediction, scaleX, scaleY)
    canvas.getContext("2d")!.drawImage(swapResult, 0, 0, canvas.width, canvas.height)
  }
  ...
  debugview = (prediction:any[]) =>{
    ...
    this.drawFacemeshRes(prediction[0], this.facemeshCanvas, this.paramss[0])
    this.drawAsciiRes(prediction[1], this.asciiCanvas)
    this.drawOpencvRes(prediction[2], this.opencvCanvas)
    this.handposeCanvas.getContext("2d")!.clearRect(0, 0, this.handposeCanvas.width, this.handposeCanvas.height)
    this.drawHandposeRes(prediction[3], this.handposeCanvas, this.paramss[3])
    this.drawSwapFace(prediction[0], this.swapCanvas, this.paramss[0])
  }
  ...
  handleResult = (prediction: any[]) => {
    ...
    this.resultCanvasRef.current!.getContext("2d")!.drawImage(baseImage, 0, 0, 
      this.resultCanvasRef.current!.width, this.resultCanvasRef.current!.height)
    

    //@ts-ignore
    const scaleX = this.originalCanvas.current!.width / this.paramss[0].processWidth
    //@ts-ignore
    const scaleY = this.originalCanvas.current!.height / this.paramss[0].processHeight
    const swapResult = this.faceswap.swapFace(this.originalCanvas.current!, prediction[0], scaleX, scaleY)
    this.resultCanvasRef.current!.getContext("2d")!.drawImage(swapResult, 0, 0, this.resultCanvasRef.current!.width, this.resultCanvasRef.current!.height)

    this.drawHandposeRes(prediction[3], this.resultCanvasRef.current!, this.paramss[3])
